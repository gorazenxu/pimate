...
- 优：扩展 `@` 上下文选择器，支持常见文本、代码、数据、Office、PDF 和图片文件。
- 安：统一文件类型识别并排除 `.obsidian`、`.trash`、`.git`、`node_modules` 等内部目录，避免内部文件出现在候选列表。
- 测：新增上下文文件筛选、路径规范化和类型图标测试。
...
- 修：AGY 终态与中断诊断更准确，避免正常完成、远程中断或网络异常被误报为用户主动停止。
- 修：AGY 用量统计将缓存读取单独计入总量和费用，避免与原始 `total_tokens` 重复或漏算。
- 优：优化长任务中工具调用和 Markdown 内容增长时的自动跟随，用户上滑阅读时不强制拉回底部。
- 文档：新增项目级审核红线与发布前检查规则。

---

## 📦 v1.1.5

- 修：AGY 已产生回复内容但终态收尾异常时，保留回复并明确区分“有回复后的收尾异常”与真正的用户停止。
- 修：切换回仍在使用的 AGY 会话时重新读取原生历史，避免最新回复只留在旧的内存快照中。
- 改：Pi 与 AGY 的次数统一按用户提交的请求统计，不再把 Pi 的多个助手用量片段当成多条消息。
- 改：Pi 首次升级统计缓存时自动重建，后续按会话文件增量更新；AGY 不回填旧 transcript。
- 新：从本版本开始记录 Pimate 实际提交的 AGY 请求，失败或没有用量回执的请求也能计数；旧 AGY 快照仍保留用量数据。
- 优：请求数界面统一显示为“请求”，历史 AGY 未回填的说明放入悬停提示，不干扰表格阅读。

---

## 📦 v1.1.3

- 修：AGY stream-json print mode 显式设置 15 分钟等待上限，避免长工具链在默认 5 分钟时被 AGY 提前终止
- 修：将 AGY 等待超时单独识别并提示为“等待本轮响应超时”，不再误报为网络中断
- 优：压缩 AGY 连续工具调用、Thought 和助手消息块的垂直间距，减少长任务日志占用屏幕空间
- 文档：README 页面示意图改为脱敏的通用界面，不包含真实 Vault 信息

---

## 📦 v1.1.2

- 新：AGY 历史会话支持在“当前空间”和“未归属”之间手动调整，提供悬停快捷按钮与右键菜单
- 新：按 Vault 持久化 AGY 会话归属覆盖标记，不删除或修改 AGY 原生历史数据
- 修：补强 AGY 历史会话的工作区校验，避免跨 Vault 误绑定、误恢复和新会话遗漏
- 优：优化 AGY 流中断、停止和进程退出状态处理，保留会话并提供手动重试，不自动重连

---

## 📦 v1.1.1

- 新：引入 Antigravity CLI（AGY）双引擎，支持在 Obsidian 内使用 Google OAuth 登录的 Gemini Agent
- 新：AGY 当前仓库历史视图、历史消息恢复与会话范围筛选
- 新：AGY Token 用量、缓存命中率、费用估算与设置页剩余用量展示
- 新：AGY 会话命名隔离与固定命名会话，降低自动命名对原生历史记录的污染
- 新：AGY 图片附件保存到 Vault 内的 `999agy对话图片` 目录，并以本地路径交给 AGY 读取
- 优：增强 AGY/Pi 进程生命周期、会话切换、历史加载及权限处理
- 优：完善双引擎设置页、会话统计、费用展示和中英文界面

---

## 🛠 Working / Uncommitted

- _当前没有未提交的项目修改；用户个人文档不纳入版本。_
